...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const animateOnScroll = function () {
    const elements = document.querySelectorAll(
      ".tile-card-1, .tile-card-2, .tile-card-3, .tile-card-4, .collection-card, .split-card, .material-card, .gradient-card",
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Only run slider logic if slides exist on this page
  ...
  // Animate stats counting — only runs if .about-stats exists on this page
    ...
    function animateStats() {
      ...
      if (sectionPosition < screenPosition) {
        statCounts.forEach((count) => {
          ...
          const suffix = count.textContent.match(/\D+$/)
            ? count.textContent.match(/\D+$/)[0]
            : "";
          const duration = 2000;
          const start = 0;
